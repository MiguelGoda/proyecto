...
router.post("/add", async (req, res) => {
  ...
  await pool.query("INSERT INTO tipo_memos set ?", [newTipoMemo]);
  req.flash("success", "El tipo memo se adicciono correctamente");
  res.redirect("/tipo_memos");
});

router.get("/", async (req, res) => {
  const tipo_memo = await pool.query("SELECT * FROM tipo_memos");
  res.render("tipo_memos/list", { tipo_memo });
});

router.get("/delete/:id", async (req, res) => {
  const { id } = req.params;
  await pool.query("DELETE FROM tipo_memos WHERE ID_TIPO_MEMO = ?", [id]);
...
router.get("/edit/:id", async (req, res) => {
  ...
  const tipo_memos = await pool.query(
    " SELECT * FROM tipo_memos WHERE ID_TIPO_MEMO = ?",
    [id]
  );
...
router.post("/edit/:id", async (req, res) => {
  ...
  await pool.query("UPDATE tipo_memos SET ? WHERE ID_TIPO_MEMO = ? ", [
    newTipoMemo,
    id
  ]);
